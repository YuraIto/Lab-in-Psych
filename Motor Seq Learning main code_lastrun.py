...
expName = 'Motor Seq Learning main code'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\DELL\\OneDrive\\Desktop\\PsychoPy\\4 Motor Sequence Learning\\Motor Seq Learning main code_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    sequential = data.TrialHandler2(
        name='sequential',
        nReps=20.0, 
        method='sequential', 
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('Mot_SeqLearning.csv'), 
        seed=None, 
    )
    thisExp.addLoop(sequential)  # add the loop to the experiment
    thisSequential = sequential.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisSequential.rgb)
    if thisSequential != None:
        for paramName in thisSequential:
            globals()[paramName] = thisSequential[paramName]
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    
    for thisSequential in sequential:
        currentLoop = sequential
        ...
        if isinstance(sequential, data.TrialHandler2) and thisSequential.thisN != sequential.thisTrial.thisN:
        ...
        # store data for sequential (TrialHandler)
        sequential.addData('key_resp.keys',key_resp.keys)
        sequential.addData('key_resp.corr', key_resp.corr)
        if key_resp.keys != None:  # we had a response
            sequential.addData('key_resp.rt', key_resp.rt)
            sequential.addData('key_resp.duration', key_resp.duration)
        # the Routine "trial" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed 20.0 repeats of 'sequential'
    ...
    thisExp.addLoop(fullRandom)  # add the loop to the experiment
    thisFullRandom = fullRandom.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisFullRandom.rgb)
    if thisFullRandom != None:
        for paramName in thisFullRandom:
            globals()[paramName] = thisFullRandom[paramName]
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    
    for thisFullRandom in fullRandom:
        currentLoop = fullRandom
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        ...
        # --- Run Routine "trial" ---
        # if trial has changed, end Routine now
        if isinstance(fullRandom, data.TrialHandler2) and thisFullRandom.thisN != fullRandom.thisTrial.thisN:
            continueRoutine = False
        trial.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
        ...
        if key_resp.keys in ['', [], None]:  # No response was made
            ...
            if str(corr_resp).lower() == 'none':
               ...
               key_resp.corr = 0;  # failed to respond (incorrectly)
        # store data for fullRandom (TrialHandler)
        fullRandom.addData('key_resp.keys',key_resp.keys)
        fullRandom.addData('key_resp.corr', key_resp.corr)
        if key_resp.keys != None:  # we had a response
            fullRandom.addData('key_resp.rt', key_resp.rt)
            fullRandom.addData('key_resp.duration', key_resp.duration)
        # the Routine "trial" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
        
    # completed 20.0 repeats of 'fullRandom'
